...
(deftest proxy-next-hop-single
  ...
  (let ((cfg (make-http-proxy-config
              :system nil
              :proxy '(("https" . "socks5h://a:1080")
                       ("http" . "http://plain:8080"))
              :no-proxy "localhost")))
    (ok (string= "socks5h://a:1080"
                 (resolve-proxy cfg "https://api.example/")))
    (ok (string= "http://plain:8080"
                 (resolve-proxy cfg "http://api.example/")))
    (ok (null (resolve-proxy cfg "https://localhost/")))
